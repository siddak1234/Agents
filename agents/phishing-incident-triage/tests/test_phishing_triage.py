...
def call(capability: str, payload: dict | None = None) -> dict:
    ...
    request = json.dumps(
        {"protocol": PROTOCOL, "capability": capability, "input": payload or {}}
    )
    proc = subprocess.run(
...
class TestContract(unittest.TestCase):
    ...
    def test_rejects_wrong_protocol(self) -> None:
        envelope = call("describe")  # valid call first
        # Now send wrong protocol
        proc = subprocess.run(
            [sys.executable, str(ENTRYPOINT)],
...
class TestTriageEmail(unittest.TestCase):
    ...
    def test_output_has_all_required_fields(self) -> None:
        ...
        output = envelope["output"]
        required = [
            "risk_score", "severity", "classification",
            "detected_indicators", "suspicious_urls",
            "sender_inconsistencies", "social_engineering_techniques",
            "authentication_findings", "evidence",
            "recommended_actions", "analyst_summary",
            "confidence", "warnings",
        ]
